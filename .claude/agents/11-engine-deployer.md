...
Ask the student: "What do you want the repo named on GitHub? Suggest: `aipm-engine-{niche-slug}`."

```bash
cd content-engine
git remote remove origin 2>/dev/null
gh repo create aipm-engine-{slug} --private --source=. --remote=origin
git add .
git commit -m "Initial deploy from AI Partner Method Local Website Builder"
